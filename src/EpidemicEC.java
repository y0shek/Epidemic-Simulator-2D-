...
public class EpidemicEC {
	...
	//Calls the Death probability and executes on all infected individuals
	killInfectedIndividuals(world);
	...
	public static void addIndividual(int[][] world, int newIndividual) {
		...
		if (world[y][x] == Config.EMPTY){
			...
			while (world[y][x] != Config.EMPTY || world [y][x] == Config.WALL){
	...
	public static void move(int[][] world) {
		for (int i = 0; i < (int)((Config.WORLD_SIZE*Config.WORLD_SIZE)/2);
				i++){
	...
	public static void swapRandomIndividuals(int[][] world) {
		...
		// Swaps the values contained by the two selected cells 
		// unless cell is a wall.
		int swapIndividualY = wrap(Config.WORLD_SIZE, y + block_y);
		int swapIndividualX = wrap(Config.WORLD_SIZE, x + block_x);

		if (world[y][x] == Config.WALL){
			return;

		}else if(world [swapIndividualY][swapIndividualX]== Config.WALL){
			return;
		...
		for (int i = 0; i < Config.WORLD_SIZE; i++) {
			for (int j = 0; j < Config.WORLD_SIZE; j++){
				if (world[i][j] != Config.UNINFECTED && world[i][j] != 
						...
						&& world[i][j] != Config.WALL) //don't decrement walls
				{world[i][j] -= 1;}
			}
		}
	}//end decrement
...
public static void spreadDisease(int[][] world) {
	...
	/**
	 * @code addWalls
	 * Add walls to the world defined by the @code Config.java
	 * @param world
	 * @param wall Assert and empty cell with Config.Wall
	 * @param wallHeight Assert a wall cell with Height attribute 
	 * defined by Config.WALL_HEIGHT
	 * @param wallSpacing Assert a wall cell with Spacing attribute 
	 * defined by Config.WALL_SPACING
	 */
	public static void addWalls (int[][] world) {
		for (int i=0; i <= Config.WALL_HEIGHT; i++){
			for (int j=0; j < Config.WORLD_SIZE; j+= Config.WALL_SPACING){
			world[i][j] = Config.WALL;
		}
		}	
	}//end addWalls
	...
	/**
	 * @code killInfectedIndividuals
	 * 
	 * Randomly kills DEATH_PROBABILITY 
	 * percentage of INFECTED (value 2 - 9) people.
	 * @param world
	 */
	public static void killInfectedIndividuals(int [][] world){
	     
		for (int i = 0; i < world.length; i++){
			for (int j = 0; j < world[0].length; j++){
		
	       if (world[wrap(Config.WORLD_SIZE, i)]
	    		   [wrap(Config.WORLD_SIZE,j)] <= Config.INFECTED 
	    		   			&& world[wrap(Config.WORLD_SIZE, i)]
	    		   			[wrap(Config.WORLD_SIZE,j)] >
	    		   				Config.RECOVERED) {
 		double probabilityDeath = Config.RANDOM.nextDouble();
 		if (probabilityDeath <= Config.DEATH_PROBABILITY)
			{world[wrap(Config.WORLD_SIZE, i)]
			[wrap(Config.WORLD_SIZE, j)] = Config.EMPTY;}
	       }
		}
		}
	} // end killInfectedIndividuals
} // end class